        ...
        <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="bg-white p-6 shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('seo.pages.agencies.update') }}" class="mt-6 grid gap-5 md:grid-cols-2">@csrf @method('PATCH')
                    ...
                    <div class="md:col-span-2"><x-input-label for="agencies_intro" value="Introduction" /><textarea id="agencies_intro" name="intro_content" data-html-editor rows="3" class="mt-1 block w-full rounded-md border-gray-300" required>{{ old('intro_content', $agencyDirectory->intro_content) }}</textarea></div>
                    <div class="md:col-span-2"><x-input-label for="agencies_bottom_content" value="Bottom SEO content" /><textarea id="agencies_bottom_content" name="bottom_content" data-html-editor rows="8" class="mt-1 block w-full rounded-md border-gray-300">{{ old('bottom_content', $agencyDirectory->bottom_content) }}</textarea></div>
