        ...
        <div class="mx-auto max-w-6xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
                <section class="space-y-6 bg-white p-6 shadow-sm sm:rounded-lg">
                    <div class="flex flex-wrap items-center gap-3"><span class="rounded-full px-3 py-1 text-xs font-bold uppercase {{ $report->priority === 'urgent' ? 'bg-red-100 text-red-800' : 'bg-gray-100' }}">{{ $report->priority }}</span><span class="text-sm font-semibold capitalize">{{ str($report->status)->replace('_', ' ') }}</span>@if($report->slaState() !== 'closed')<span class="rounded-full px-3 py-1 text-xs font-bold {{ $report->slaState() === 'overdue' ? 'bg-red-100 text-red-800' : ($report->slaState() === 'due_soon' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800') }}">{{ $report->slaState() === 'overdue' ? 'Overdue by ' : 'Due in ' }}{{ $report->slaDeadline()->diffForHumans(now(), \Carbon\CarbonInterface::DIFF_ABSOLUTE, true, 2) }}</span>@endif<span class="text-sm text-gray-500">{{ $report->public_id }}</span></div>
                    <div><p class="text-sm text-gray-500">Category</p><h3 class="text-xl font-bold">{{ $report->categoryLabel() }}</h3></div>
                    <div><p class="text-sm text-gray-500">Reporter follow-up</p><p class="font-medium">{{ $report->reporter_email ?? 'Redacted after retention period' }}</p><p class="text-xs text-gray-500">{{ $report->reporter?->email ? 'Registered account' : ($report->reporter_email ? 'Guest report' : 'Closed-case personal data removed') }}</p></div>
