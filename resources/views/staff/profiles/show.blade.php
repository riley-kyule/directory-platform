        ...
        <div class="mx-auto grid max-w-6xl gap-6 px-4 sm:px-6 lg:grid-cols-3 lg:px-8">
            <section class="space-y-6 lg:col-span-2">
                <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                    ...
                    <div class="mt-5"><p class="text-sm text-gray-500">Photos</p><div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3">@forelse ($packageRequest->profile->images as $image)@if (in_array($image->status, ['pending_review', 'reviewed', 'approved']) && isset($image->derivatives['thumb']))<img src="{{ route('profiles.media.preview', [$packageRequest->profile, $image, 'thumb']) }}" alt="Submitted media" class="aspect-[4/5] w-full rounded-md object-cover">@else<div class="flex aspect-[4/5] items-center justify-center rounded-md bg-gray-100 p-2 text-center text-xs text-gray-500">{{ str($image->status)->replace('_', ' ')->title() }}</div>@endif @empty<p class="col-span-full text-sm text-red-700">No photos submitted.</p>@endforelse</div></div>
                    @if ($packageRequest->profile->videos->isNotEmpty())
                        <div class="mt-5"><p class="text-sm text-gray-500">Videos</p><div class="mt-2 grid gap-3 sm:grid-cols-2">@foreach ($packageRequest->profile->videos as $video)@if (in_array($video->status, ['pending_review', 'reviewed', 'approved']))<video controls preload="metadata" playsinline class="aspect-video w-full rounded-md bg-black"><source src="{{ route('profiles.media.videos.preview', [$packageRequest->profile, $video]) }}" type="{{ $video->mime_type }}"></video>@else<div class="flex aspect-video items-center justify-center rounded-md bg-gray-100 p-2 text-center text-xs text-gray-500">{{ str($video->status)->replace('_', ' ')->title() }}</div>@endif @endforeach</div></div>
                    @endif
                </div>
            </section>
            ...
            <aside class="bg-white p-6 shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('staff.profiles.update', $packageRequest) }}" class="mt-5 space-y-5">
                    ...
                    <div><x-input-label for="reason" value="Decision note (optional)" /><textarea id="reason" name="reason" rows="5" class="mt-1 block w-full rounded-md border-gray-300">{{ old('reason') }}</textarea><x-input-error :messages="$errors->get('reason')" class="mt-2" /></div>
                    @if ($canOverrideRequirements)
                        <label class="block rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-900">
                            <span class="flex items-start gap-3">
                                ...
                                <span><strong>Admin/CSR override</strong><br>Approve even if verification or processed-media requirements are missing. Missing verification checks will be marked verified as a permanent, attributable staff override recorded in the audit log.</span>
                            </span>
                        </label>
                    @endif
