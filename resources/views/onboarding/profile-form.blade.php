    ...
    <div class="py-12" x-data="{ additionalOpen: {{ $errors->hasAny(['hair_color_option_id', 'hair_length_option_id', 'height_cm', 'weight_kg', 'smoker', 'language_ids', 'website_url', 'instagram_handle', 'snapchat_handle', 'tiktok_handle']) ? 'true' : 'false' }}, telegramPhoneEnabled: {{ old('telegram_phone_enabled') ? 'true' : 'false' }} }">
        <form method="POST" action="{{ route('onboarding.profiles.store') }}" class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="grid gap-5 bg-white p-6 shadow-sm sm:rounded-lg md:grid-cols-2">
                ...
                <div><x-input-label for="phone" value="Phone number" /><x-text-input id="phone" name="phone" type="tel" class="mt-1 block w-full" :value="old('phone')" placeholder="+254..." required /><p class="mt-1 text-xs text-gray-500">Use international format with country code, for example +254700000000.</p><x-input-error :messages="$errors->get('phone')" class="mt-2" /></div>
