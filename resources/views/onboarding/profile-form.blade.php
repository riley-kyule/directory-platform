...
<x-app-layout>
    ...
    <div class="py-12" x-data="{ additionalOpen: {{ $errors->hasAny(['hair_color_option_id', 'hair_length_option_id', 'height_cm', 'weight_kg', 'smoker', 'language_ids', 'website_url', 'instagram_handle', 'snapchat_handle', 'tiktok_handle']) ? 'true' : 'false' }}, telegramPhoneEnabled: {{ $value('telegram_phone_enabled') ? 'true' : 'false' }} }">
        <form method="POST" action="{{ $editing ? route('provider.profiles.update', $profile) : route('onboarding.profiles.store') }}" class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="grid gap-5 bg-white p-6 shadow-sm sm:rounded-lg md:grid-cols-2">
                ...
                <div><x-input-label for="display_name" value="Name" /><x-text-input id="display_name" name="display_name" class="mt-1 block w-full" :value="$value('display_name')" required /><x-input-error :messages="$errors->get('display_name')" class="mt-2" /></div>
                <div><x-input-label for="phone" value="Phone number" /><x-text-input id="phone" name="phone" type="tel" class="mt-1 block w-full" :value="$value('phone')" placeholder="+254..." required /><p class="mt-1 text-xs text-gray-500">Use international format with country code, for example +254700000000.</p><x-input-error :messages="$errors->get('phone')" class="mt-2" /></div>
                <fieldset class="md:col-span-2"><legend class="text-sm font-medium text-gray-700">Phone availability</legend><div class="mt-2 flex flex-wrap gap-5"><label><input type="checkbox" name="whatsapp_enabled" value="1" @checked($value('whatsapp_enabled'))> Available on WhatsApp</label><label><input type="checkbox" name="telegram_phone_enabled" value="1" x-model="telegramPhoneEnabled" @checked($value('telegram_phone_enabled'))> Available on Telegram</label></div></fieldset>
                ...
                <div><x-input-label for="date_of_birth" value="Date of birth" /><x-text-input id="date_of_birth" name="date_of_birth" type="date" max="{{ now()->subYears(18)->toDateString() }}" class="mt-1 block w-full disabled:bg-gray-100" :value="$value('date_of_birth')" :disabled="$editing" :required="! $editing" />@if ($editing)<p class="mt-1 text-xs text-gray-500">Contact staff to change this sensitive field.</p>@endif<x-input-error :messages="$errors->get('date_of_birth')" class="mt-2" /></div>

                <fieldset><legend class="text-sm font-medium text-gray-700">Availability</legend><div class="mt-2 flex gap-5"><label><input type="checkbox" name="allows_incall" value="1" @checked($value('allows_incall'))> Incall</label><label><input type="checkbox" name="allows_outcall" value="1" @checked($value('allows_outcall'))> Outcall</label></div><x-input-error :messages="$errors->get('availability')" class="mt-2" /></fieldset>

                <fieldset class="md:col-span-2"><legend class="text-sm font-medium text-gray-700">Services</legend><div class="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3">@foreach ($taxonomies['service'] ?? [] as $option)<label><input type="checkbox" name="service_ids[]" value="{{ $option->id }}" @checked(in_array($option->id, $value('service_ids', [])))> {{ $option->label }}</label>@endforeach</div><x-input-error :messages="$errors->get('service_ids')" class="mt-2" /></fieldset>
            ...
            <section class="grid gap-5 bg-white p-6 shadow-sm sm:rounded-lg md:grid-cols-3">
                ...
                <div><x-input-label for="rate_currency" value="Currency" /><x-text-input id="rate_currency" name="rate_currency" maxlength="3" class="mt-1 block w-full uppercase" :value="$value('rate_currency')" placeholder="KES" /></div>
                <div><x-input-label for="rate_period_option_id" value="Period" /><select id="rate_period_option_id" name="rate_period_option_id" class="mt-1 block w-full rounded-md border-gray-300"><option value="">Choose period</option>@foreach ($taxonomies['rate_period'] ?? [] as $option)<option value="{{ $option->id }}" @selected($value('rate_period_option_id') == $option->id)>{{ $option->label }}</option>@endforeach</select></div>
                <div><x-input-label for="rate_price" value="Price" /><x-text-input id="rate_price" name="rate_price" type="number" min="0" step="0.01" class="mt-1 block w-full" :value="$value('rate_price')" /></div>
            </section>

            @unless ($editing)
                <section class="bg-white p-6 shadow-sm sm:rounded-lg">
                    <h3 class="text-lg font-semibold text-gray-900">Choose package</h3>
                    <div class="mt-4 grid gap-4 sm:grid-cols-3">@foreach ($packages as $package)<label class="rounded-lg border p-4"><input type="radio" name="requested_package_id" value="{{ $package->id }}" @checked($value('requested_package_id') == $package->id) required> <span class="font-semibold">{{ $package->name }}</span><span class="mt-1 block text-sm text-gray-600">Up to {{ $package->image_limit }} images</span></label>@endforeach</div>
